...
```{r}
ezANOVA(data=sub_p2 %>% filter(roi %in% c('ca1-body', 'ca23dg-body') & round == 'early' & segment == 'same'),
        wid=sub,
        within = .(valid, roi),
...
ggplot(p4 %>% filter(round == 'early' & roi %in% c('ca1-body', 'ca23dg-body')), aes(x = segment, y = m, fill = roi)) +
      geom_hline(yintercept = 0, linetype="longdash") +
      geom_bar(stat = 'identity', alpha = 0.5, position = position_dodge()) + 
      geom_errorbar(aes(ymin = m-se, ymax = m+se), position = position_dodge(width = 0.8), width=0.5) +
      labs(y = 'within_pair - across_pair')+ 
...
ggplot(p4 %>% filter(round != 'others'), aes(x = segment, y = m, fill = valid)) +
      geom_hline(yintercept = 0, linetype="longdash") +
      geom_bar(stat = 'identity', alpha = 0.5, position = position_dodge()) + 
      geom_errorbar(aes(ymin = m-se, ymax = m+se), position = position_dodge(width = 0.8), width=0.5) +
      labs(y = 'within_pair - across_pair')+ 
...
```{r}
ezANOVA(data=sub_p4 %>% filter(roi %in% c('ca1-body', 'ca23dg-body') & round == 'early' & valid == 'valid-valid'),
        wid=sub,
        within = .(roi, segment),
        dv = cor)$ANOVA %>% knitr::kable()

ezANOVA(data=sub_p4 %>% filter(roi %in% c('ca1-body', 'ca23dg-body') & round == 'early' & valid == 'valid-invalid'),
        wid=sub,
        within = .(roi, segment),
        dv = cor)$ANOVA %>% knitr::kable()
