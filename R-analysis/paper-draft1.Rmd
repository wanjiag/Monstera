...
sub_p2 = sub_p1 %>% 
  ...
  #filter(!within_trial_TR %in% c(6, 12, 18)) %>% 
...
ANOVA across ROI (ca23dg & ca1), segment, and valid

```{r eval=FALSE, include=FALSE}
ezANOVA(data=sub_p2 %>% filter(roi %in% c('ca23dg-body', 'ca1-body')),
        wid=sub,
...
ezANOVA(data=sub_p2 %>% filter(roi %in% c('ca1-body', 'ca23dg-body') & valid == 'valid-valid'),
        wid=sub,
        within = .(roi, segment),
        dv = m)$ANOVA %>% knitr::kable()
```
...
m1 <- lmer(m ~ segment * valid * roi + (1 | sub), sub_p2 %>% filter(roi %in% c('ca23dg-body', 'ca1-body')))
anova(m1)
...
```{r}
m1 <- lmer(m ~ segment * valid + (1 | sub), sub_p2 %>% filter(roi %in% c('ca23dg-body')))
anova(m1)
summary(m1)$coefficients

m2 <- lmer(m ~ segment * valid + (1 | sub), sub_p2 %>% filter(roi %in% c('ca23dg-body') & segment %in% c('same', 'early-similar')))
anova(m2)
summary(m2)$coefficients

m3 <- lmer(m ~ segment * valid + (1 | sub), sub_p2 %>% filter(roi %in% c('ca1-body')))
...
left_join(sub_together %>% mutate(sub = as.numeric(sub)), 
  ...
  #filter(within_trial_TR >= min - 5 & within_trial_TR <= max + 5 ) %>% 
...
p3$segment = factor(p3$segment,levels = c('pre', 'MoI', 'post'))
...
ggplot(p3, aes(x = segment, y = m, fill = valid)) +
      ...
      geom_point(data = sub_p3, alpha = 0.3,
...
ggplot(p3 %>% filter(valid == 'valid-valid' & roi %in% c('ca23dg-body', 'ca1-body')), aes(x = segment, y = m, fill = roi)) +
      geom_hline(yintercept = 0, linetype="longdash") +
      geom_bar(stat = 'identity', alpha = 0.5, position = position_dodge()) + 
      geom_errorbar(aes(ymin = m-se, ymax = m+se), position = position_dodge(width = 0.8), width=0.5) +
      geom_point(data = sub_p3 %>% filter(roi %in% c('ca23dg-body', 'ca1-body'), valid == 'valid-valid'), alpha = 0.3,
...
fig1 = p1 %>% filter(roi %in% c('ca23dg-body')) %>% 
  ...
  scale_fill_manual(values = cbPalette[7:8],
                    labels = c('expect different', 'expect same'))+ 
  scale_color_manual(values = cbPalette[7:8],
                     labels = c('expect different', 'expect same'))+
...
cowplot::plot_grid(fig1, fig2, ncol=1, align='v', rel_heights = c(2/3, 1/3))
...
ggplot(p4 %>% filter(roi %in% c('ca23dg-body', 'ca1-body'), valid == 'valid-valid'), aes(x = segment, y = m, fill = roi)) +
      geom_hline(yintercept = 0, linetype="longdash") +
      geom_bar(stat = 'identity', alpha = 0.5, position = position_dodge()) + 
      geom_errorbar(aes(ymin = m-se, ymax = m+se), position = position_dodge(width = 0.8), width=0.5) +
      labs(y = 'Similarity Scores')+ 
...
ezANOVA(data=sub_p4 %>% filter(roi %in% c('ca23dg-body', 'ca1-body') & valid == 'valid-valid'),
        wid=sub,
        within = .(roi, segment),
        dv = cor)$ANOVA %>% knitr::kable()

ezANOVA(data=sub_p4 %>% filter(roi %in% c('ca1-body') & valid == 'valid-valid'),
        wid=sub,
        within = .(segment),
        dv = cor)$ANOVA %>% knitr::kable()
...
#### lmer
```{r}
m5 <- lmer(cor ~ segment * roi + (1 | sub), sub_p4 %>% filter(roi %in% c('ca23dg-body', 'ca1-body') & valid == 'valid-valid'))
anova(m5)


m6 <- lmer(cor ~ segment + (1 | sub), sub_p4 %>% filter(roi %in% c('ca23dg-body') & valid == 'valid-valid'))
anova(m6)

m7 <- lmer(cor ~ segment + (1 | sub), sub_p4 %>% filter(roi %in% c('ca1-body') & valid == 'valid-valid'))
anova(m7)

m8 <- lmer(cor ~ segment * valid + (1 | sub), sub_p4 %>% filter(roi %in% c('ca23dg-body')))
anova(m8)
```
