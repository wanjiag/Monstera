...
making_2_plots <- function(df){
  ...
  p2 = tmp_sub %>% ggplot(aes(x = within_trial_TR, y = wi_wp_m, color = valid)) +
    geom_vline(xintercept=6) +
    geom_vline(xintercept=18) +
    ...
    p4 = tmp3 %>% group_by(segment, valid, roi) %>% 
      ...
      geom_bar(stat = 'identity', alpha = 0.5, position = position_dodge()) + 
      geom_errorbar(aes(ymin = m-se, ymax = m+se), position = position_dodge(width = 0.8), width=0.5) +
      geom_point(data = tmp3, aes(x = segment, y = wi_wp_m, color = valid, group = valid), 
                  position=position_jitterdodge(
                    jitter.width = 0.2,
                    dodge.width = 0.75)) + 
      labs(y = 'within_item - within_pair')+ 
...
```{r 0522 presentation figures, eval=FALSE, include=FALSE}
tmp = df %>% mutate(wp_ap = within_pair - across_pair,
                      wi_wp = within_item - within_pair,
         segment = factor(segment, levels = c('same', 'overlapping', 'non-overlapping'))) %>% 
...
unique(all_sub_phase_df_unnest$phase)
unique(all_sub_phase_df_unnest$valid)
...
tmp = all_sub_phase_df_unnest %>% 
  ...
  tmp2_mean %>% group_by(segment, roi, valid, type, phase) %>% 
      summarise(m = mean(mean),
                se = sd(mean)/sqrt(n()),
                n = n()
                ) %>% 
      ...
      geom_bar(stat = 'identity', alpha = 0.5, position = position_dodge()) + 
      geom_errorbar(aes(ymin = m-se, ymax = m+se), position = position_dodge(width = 0.9), width=0.5) +
      geom_point(data = tmp2_mean, aes(x = segment, y = mean, color = phase, group = phase), 
                  position=position_jitterdodge(
                    jitter.width = 0.2,
                    dodge.width = 0.9)) + 
      labs(y = 'within_item-within_pair')+ 
...
output = making_2_plots(all_sub_plot)
