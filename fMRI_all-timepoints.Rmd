...
re_process = FALSE
file_name =  './csv_files/R-tmp/all-sub-df_n=11_0921.RDS'
...
One-sample t-test for CA23DG at Same segment
```{r}
t.test(data %>% filter(roi == 'ca23dg-body' & segment == 'same') %>% .$wp_ap_m)
```

One-sample t-test for CA23DG at Overlapping segment
```{r}
t.test(data %>% filter(roi == 'ca23dg-body' & segment == 'overlapping') %>% .$wp_ap_m)

```

One-sample t-test for CA23DG at Non-overlapping segment
```{r}
t.test(data %>% filter(roi == 'ca23dg-body' & segment == 'non-overlapping') %>% .$wp_ap_m)
```
